    ...
    async def lifespan(app: FastAPI):
        ...
        async def _run_one_job(job_id: str, req: GenerateMusicRequest) -> None:
            ...
            def _blocking_generate() -> Dict[str, Any]:
                ...
                if client_title and not metas_out.get("title"):
                ...
                # Build a base payload (job-level)
                base_payload = {
                    "job_id": job_id,
                    ...
                    # Original user input (you already store in metas_out too)
                ...
                # Write per-audio sidecars (include per-file seed)
                for idx, audio in enumerate(result.audios):
                    p = audio.get("path")
                    if not p:
                        continue
                    per_seed = (audio.get("params") or {}).get("seed")
                    per_payload = dict(base_payload)
                    per_payload["audio_index"] = idx
                    per_payload["audio_path"] = p
                    per_payload["seed"] = per_seed
                    try:
                        _write_audio_sidecar_json(p, per_payload)
                    except Exception as e:
                        print(f"[API Server] Warning: failed writing sidecar for {p}: {e}")
...
def _sidecar_meta_path(audio_path: str) -> str:
    # "song.mp3" -> "song.mp3.json" (easy to derive from client)
    return audio_path + ".json"

def _write_audio_sidecar_json(audio_path: str, payload: dict) -> str:
    meta_path = _sidecar_meta_path(audio_path)
